import { LAYOUT_STYLE_PROPS, TEXT_STYLE_PROPS, VIEW_STYLE_PROPS } from './constants';
...
const groupStyleObject = (style = {}) => {
  return Object.entries(style).reduce((acc, [key, value]) => {
    const [group] = Object.entries(styleGroups).find(([, groupProps]) => groupProps.includes(key));
    return group
      ? {
          ...acc,
          [group]: {
    ...
    return mapped.reduce((acc, currentGroup) => {
      return Object.assign(
        ...
        ...Object.keys(styleGroups).map((key) =>
          currentGroup[key]
            ? {
                [key]: acc[key] ? [...acc[key], currentGroup[key]] : [currentGroup[key]],
              }
            : undefined,
        ),
      );
